...
$arrcolor=array(1=>"e0f7fa",2=>"#F5DA81",3=>"#A9F5A9",4=>"#A9F4F3",5=>"#CFCEF7",6=>"#D1A9F4",7=>"#F5A8F3",8=>"#80DBF5",9=>"#9BFE2F",10=>"#9BFE2F");
...
<div class="contenidoinfo">
    <div class="texto-transversal">
      ...
      <p>'.$row->resultadosaprendizaje.'.</p>
			...
			$data=$data.'<br><p>';
			if(isset($row->silabopdf)){
			    $data=$data.'[<a href="https://repositorioutlvte.org/Repositorio/'.$row->silabopdf.'"  '.$disable0.'><i class="fas fa-file-pdf" style="font-size:24px" ></i> <span style="color:'.$color0.'" >Sillabus</span></a>] - ';
            }
...
foreach($jornadadocente as $rowj){
	...
	if(strpos($row->estadoevento,"TERMINADO")!==false || strpos($row->estadoevento,"PRÓXIMO A INICIAR")!==false  ){
		...
		$data=$data.'<br> <p><b>ESTADO : </b><span style="color:green">'.$row->estadoevento.'.</span></p>';
                ...
                <div class="btn-group">
                  <button type="button" class="btn btn-sm btn-outline-secondary" onclick="location.href=\'http://5.161.176.197/sica/index.php/login/validarcorreo?idevento='.$row->idevento.'\'" '.$disabled.' >Inscribete</button>
