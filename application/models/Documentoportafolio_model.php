...
class Documentoportafolio_model extends CI_model {
	...
	function lista_documentoportafoliosA(){
 	...
 	function documentoportafoliodocente( $iddocente,$idperiodoacademico){

 		$documentoportafolio = $this->db->query('select * from documentoportafolio2 where iddocente="'. $iddocente.'" and idperiodoacademico="'. $idperiodoacademico.' "  order by iddocente   ');
 		return $documentoportafolio;
 	}
  ...
  public function get_documentosportafolio($idportafolio) {
        ...
        $query = $this->db->get('documentoportafolio4'); // Suponiendo que la tabla se llama 'documentos'
