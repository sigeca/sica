...
class Portafolio_model extends CI_model {
	...
	//	 $portafolio= $this->db->order_by("idpersona","idperiodoacademico")->get('portafolio2');
 		...
 		$portafolio = $this->db->query('select port.idportafolio,pele.idperiodoacademico, port.idpersona,pele.nombrecorto as elperiodo,concat(COALESCE(pers.apellidos,\'\'),"  ",COALESCE(pers.nombres,\'\')) as lapersona,dopo.iddocumentoportafolio,docu.iddocumento,docu.asunto,docu.fechaelaboracion,docu.archivopdf,dire.iddirectorio, dire.ruta,orde.idordenador, orde.nombre as elordenador, dclp.nombre as elchklstdocumento, dclp.orden  from portafolio port, periodoacademico pele, persona pers, documento docu, documentoportafolio dopo,  directorio dire,ordenador orde, documentochklstportafolio dclp  where port.idperiodoacademico=pele.idperiodoacademico and port.idpersona=pers.idpersona and port.idportafolio=dopo.idportafolio and dopo.iddocumento=docu.iddocumento  and docu.iddirectorio=dire.iddirectorio and docu.idordenador=orde.idordenador and dopo.iddocumentochklstportafolio=dclp.iddocumentochklstportafolio and pers.idpersona="'.$idpersona.'" and pele.idperiodoacademico="'.$idperiodoacademico.'" order by dclp.orden, elchklstdocumento'); 
