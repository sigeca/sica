...
create view participante1 as select pa.idparticipante,pa.idnivelparticipante,pa.idpersona,ev.idevento,ev.titulo as elevento,concat(pe.apellidos," ",pe.nombres) as nombres,pe.cedula,doc.archivopdf,pa.grupoletra,(select  co.nombre from correo co where co.idpersona=pa.idpersona and co.idcorreo_estado=1 limit 1) as correo,pa.iddocumento,pa.idparticipanteestado  from participante0 pa,evento ev,persona pe,documento doc where pa.idpersona=pe.idpersona and pa.idevento=ev.idevento and pa.iddocumento=doc.iddocumento
UNION 

select pa.idparticipante,pa.idnivelparticipante,pa.idpersona,ev.idevento,ev.titulo as elevento,concat(pe.apellidos," ",pe.nombres) as nombres,pe.cedula," " as archivopdf,pa.grupoletra,(select  co.nombre from correo co where co.idpersona=pa.idpersona and co.idcorreo_estado=1 limit 1) as correo,pa.iddocumento,pa.idparticipanteestado   from participante0 pa,evento ev,persona pe,documento doc where pa.idpersona=pe.idpersona and pa.idevento=ev.idevento and pa.idevento=ev.idevento and  pa.iddocumento=0;
